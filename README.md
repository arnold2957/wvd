...
- 初次加载遇到黑屏是正常现象. 遇到google登录弹窗按返回即可.
...
- "智能开箱"是基于图像识别和拟合三角波来做预测的.
	- 因为是拟合, 所以极度依赖网络环境的稳定. 如果开箱时出现网络问题, **无法点击"retry"**, 会导致卡死. 请见谅.
	- 目前是通过20张截图来预测的. 之后会换更加快速的实现方案.
...
- 旅店休息间隔控制在一次栈桥之行中, 会重复执行几次"击杀前2波"的操作之后再回城. 例如, 如果休息间隔=- 那么会重复2轮, 一共2x2=4次战斗和4次aoe释放.
- 请确保第七区的地图全部点亮.
