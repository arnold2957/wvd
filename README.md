...
- 由于游戏会在死亡或重启后丢失角色的上一次行动, 会导致游戏内设置"重复上一次"失效, 因此会在每次死亡或重启游戏后重置战斗方案.
- 如果角色在方案里, 那么会按照方案的设置释放技能且暂时删除这一条目. 如果角色不在方案里, 那么会双击自动, 此时遵循游戏内对该角色的自动战斗方案.
- 下面是具体的例子:
	- 我想让柚子复读千紫万红3:
		策略写一条"柚子, 千紫万红, 3", 游戏内设置柚子的自动方案为"重复上一次".
	- 我想让叶卡每场战斗仅释放一次aoe, 并在之后进行自动攻击:
	  	策略写一条"叶卡, aoe, 1", 脚本开启"每场战斗重置". 游戏内设置叶卡的自动方案为"攻击".
   		脚本释放一次叶卡aoe后, 策略为空, 此时执行游戏内的自动方案, 游戏内的自动方案为攻击, 那么叶卡会持续攻击.
	- 我想让面具人进行动静裂地循环:
		脚本写2*n条, 第一条"面具, 动静, 1". 第二条"面具, 裂地, 1", 然后按照你的喜好重复若干次.
