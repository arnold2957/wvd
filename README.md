...
### 模拟器设置
- 脚本左上角的"模拟器路径"一般是C:\Program Files\BlueStacks_nxt\HD-Adb.exe. 该路径是你安装蓝叠模拟器的路径.
...
- 稳健起见, 需要1速aoe来秒杀魅魔, 不然会有较高几率翻车. 因为伤害问题, 需要后排配爱丽丝+前排光环工具人. 同时因为续航问题, 可以考虑使用2组.
- 参考阵容:
	  夏莉莉+米拉娜(续航优秀, 可开锁, 但前排补刀的伤害略低) > 叶卡+艾莉赛(可以防止偷袭同时单光环足矣, 但是续航不够) > 亚当+黛博拉(续航和伤害都不足) > 亚当+阿贝(先驱斩浪费时间而且无效伤害)
